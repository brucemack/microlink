...
The SSRC is followed by 8 bytes of unknown meaning.  I see the EchoLink client sending these consistently for session initiation:

        E1 CA 00 18 00 00 00 00

I suspect that the last 4 zeros are a repeat of the SSRC.  Zero for the EchoLink client but non-zero for the ECHOTEST station (see below).
...
* A type 2 token is sent that contains the FCC callsign (padded with spaces to 15 characters) and the user's real name.  So in my case I see:
...
* A type 8 token with an unknown meaning. The values sent by the official EchoLink client are consistent over time.  I see these bytes being sent (in hex):

        01 50 35 31 39 38

* A type 8 token with an unknown meaning. The values sent by the official EchoLink client are consistent over time. Is see these byte being sent (in hex):

        01 44 30

Finally, optional padding is added to bring the last token up to the next 4-byte boundary (per SDES termination requirement), and then the entire packet is padded again following the RFC requirement.
...
This is followed by 8 bytes of text that contain text of unknown meaning.  The standards documents suggest that this contains some arbitrary text with the "reason for leaving."  There are no delimiters or length bytes included:
...
These packets contain no header information and appear to be plain text with \n and null termination.  For example:
...
* The official EchoLink client has 4 additional bytes following the null termination of the text.  The meaning of these bytes is unknown.  The oNDATA messages that are received from the ECHOTEST station do not have these extra bytes. I see these being generated by EchoLink:
